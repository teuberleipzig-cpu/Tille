...
import * as releases from './modules/releases.js';
...
async function boot() {
  console.info('[ResidentPortal]', PORTAL_VERSION);
  initModules();
  try {
    await loadResidentForLogin();
    showScreen('loginScreen');
    setStatus(`${PORTAL_VERSION} geladen. Bitte Code und GitHub-Token eingeben.`, 'ok');
  } catch (error) {
    showScreen('loginScreen');
