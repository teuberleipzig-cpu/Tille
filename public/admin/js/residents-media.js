...
(function(){
  ...
  function normalizePhotos(r){
    ...
    r.photoList=r.photoList.map(p=>typeof p==='string'?{url:p,caption:''}:{url:p.url||p.src||p.imageUrl||'',caption:p.caption||p.alt||''});
    return r.photoList;
  }
  ...
  function injectMediaUi(){
    ...
    wrap.innerHTML='<div class="notice" style="margin-top:16px">Fotos werden aktuell als URL/Data-URL in <code>photoList</code> gespeichert. Embeds werden zeilenweise in <code>embeds</code> gespeichert.</div><div class="head" style="margin-top:18px"><div><b>Fotos</b><div class="muted">Mehrere Fotos pro Resident, 16:9 gecroppt.</div></div><div class="tools"><button class="tool" id="addResidentPhotoUrlBtn">+ Foto-URL</button><button class="tool" id="uploadResidentPhotoBtn">Foto hochladen</button><input id="residentPhotoFile" type="file" accept="image/*" class="hidden"></div></div><div id="residentPhotosList" class="resident-photo-grid"></div><div class="field full" style="margin-top:18px"><label class="label">Media Embeds, ein Embed oder Link pro Zeile</label><textarea class="input resident-embeds-textarea" id="residentEmbedsTextarea" placeholder="SoundCloud / YouTube / Bandcamp Embed oder URL"></textarea></div>';
    panel.appendChild(wrap);
    $('addResidentPhotoUrlBtn').onclick=()=>{const r=currentResident();if(!r)return;normalizePhotos(r).push({url:'',caption:''});markDirty();renderResidentMedia();};
    $('uploadResidentPhotoBtn').onclick=()=>$('residentPhotoFile').click();
    $('residentPhotoFile').onchange=handleResidentPhotoUpload;
  ...
  function renderResidentMedia(){
    ...
    listEl.innerHTML=photos.map((p,i)=>'<div class="resident-photo-card"><img src="'+esc(p.url||'')+'" alt=""><div class="field"><label class="label">Bild-URL / Data-URL</label><textarea class="input" data-photo-url="'+i+'">'+esc(p.url||'')+'</textarea></div><div class="field"><label class="label">Caption / Alt</label><textarea class="input" data-photo-caption="'+i+'">'+esc(p.caption||'')+'</textarea></div><div class="tools" style="margin-top:10px"><button class="tool" data-photo-move="'+i+':-1">↑</button><button class="tool" data-photo-move="'+i+':1">↓</button><button class="tool danger" data-photo-remove="'+i+'">Löschen</button></div></div>').join('')||'<p class="muted">Noch keine Fotos.</p>';
    wirePhotos();
  }
  function wirePhotos(){
    ...
    document.querySelectorAll('[data-photo-caption]').forEach(el=>{el.oninput=()=>{const r=currentResident();if(!r)return;normalizePhotos(r)[Number(el.dataset.photoCaption)].caption=el.value;markDirty();};});
  ...
  function readResidentMedia(){
    ...
    document.querySelectorAll('[data-photo-caption]').forEach(el=>{if(photos[Number(el.dataset.photoCaption)]) photos[Number(el.dataset.photoCaption)].caption=el.value;});
  ...
  function handleResidentPhotoUpload(e){
    ...
    cropResidentPhoto(file).then(dataUrl=>{
      normalizePhotos(currentResident()).push({url:dataUrl,caption:''});
      markDirty();
      renderResidentMedia();
