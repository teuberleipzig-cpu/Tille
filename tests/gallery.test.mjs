...
test('cache-busting references are current', async () => {
  ...
  for (const name of names) { const html = await readFile(new URL(name, root), 'utf8'); assert.doesNotMatch(html, /site-navigation\.js\?v=site-navigation-[1-4]/); assert.match(html, /site-navigation\.js\?v=site-navigation-5/); }
