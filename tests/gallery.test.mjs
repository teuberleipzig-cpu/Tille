...
test('fresh Gallery load clears stale pending deletes', async () => {
  const admin = await readFile(new URL('../public/admin/js/features/gallery/gallery.js', import.meta.url), 'utf8');
  assert.match(admin, /loadedSha = file\.sha; window\.AdminStaging\.resetPending\('gallery'\); pendingMediaDeletes\.clear\(\)/);
});
...
test('cache-busting references are current', async () => {
  ...
  for (const name of names) { const html = await readFile(new URL(name, root), 'utf8'); assert.doesNotMatch(html, /site-navigation\.js\?v=site-navigation-[1-7]/); assert.match(html, /site-navigation\.js\?v=site-navigation-8/); }
  const admin = await readFile(new URL('../public/admin/index.html', import.meta.url), 'utf8'); assert.match(admin, /gallery\.js\?v=admin-staging-1/);
});
