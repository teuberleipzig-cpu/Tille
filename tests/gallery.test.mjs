...
test('cache-busting references are current', async () => {
  ...
  for (const name of names) { const html = await readFile(new URL(name, root), 'utf8'); assert.doesNotMatch(html, /site-navigation\.js\?v=site-navigation-[123]/); assert.match(html, /site-navigation\.js\?v=site-navigation-4/); }
